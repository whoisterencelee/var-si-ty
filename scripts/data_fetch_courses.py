    ...
    with urllib.request.urlopen(req) as resp:
        ...
        try:
            ...
            try:
                ...
                text = data.decode('utf-8', errors='ignore')  # Last resort
        try:
            return json.loads(text)
        except json.JSONDecodeError:
            # If JSON parsing fails, return the raw text
            print(f"Warning: Could not parse JSON from {url}, returning raw text")
            return text
        ...
        if os.path.exists(OUT_FILE):
            ...
            def process_batch(batch):
                ...
                for ev in batch:
                    ...
                    if courseCode in idx_by_id:
                        # Update the existing record in-place with the newest data, then stop
                        existing_list[idx_by_id[courseCode]] = ev
                        updated_and_stopped = True
                        print("Encountered duplicate courseCode={}, updated record and stopping.".format(courseCode))
                        break
                    else:
                        # New event: prepend to keep newest-first ordering
                        existing_list.insert(0, ev)
                        # Track index for potential later lookups
                        idx_by_id[courseCode] = 0  # inserted at front
